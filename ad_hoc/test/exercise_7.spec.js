...
const { isDuplicate, isDuplicate2, isDuplicate3, isDuplicate4, checkDuplicate } = require('../src/exercise_7/is_duplicate')
...
describe('Exercise 7: is duplicate.', () => {
  ...
  describe('Method 5', () => {
    it('should return true if repeated values.', () => {
      const arr = [1,2,3,3,4,5]
      const expectedValue = true
      console.time('\tisDuplicate true version 5')
      const actualValue = checkDuplicate(arr)
      console.timeEnd('\tisDuplicate true version 5')
      expect(actualValue).to.eql(expectedValue)
    })
    ...
    it('should return false if no value is repeated.', () => {
      const arr = [1,2,3,4,5]
      const expectedValue = false
      console.time('\tisDuplicate false version 5')
      const actualValue = checkDuplicate(arr)
      console.timeEnd('\tisDuplicate false version 5')
      expect(actualValue).to.eql(expectedValue)
    })
  })
})
